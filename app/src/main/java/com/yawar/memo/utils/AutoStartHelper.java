...
//        sharedpreferences = context.getSharedPreferences(DashBord.MyPREFERENCES, MODE_PRIVATE);
//        editor = context.getSharedPreferences(DashBord.MyPREFERENCES, MODE_PRIVATE).edit();
        String build_info = Build.BRAND.toLowerCase();
        System.out.println("build_info"+build_info);
        switch (build_info) {
            case BRAND_ASUS:
    ...
    private void showAlert(Context context, DialogInterface.OnClickListener onClickListener) {
        ...
        new AlertDialog.Builder(context).setTitle(R.string.allow_auto_start)
                .setMessage(R.string.aloow_auto_start_message)
                .setPositiveButton(R.string.allow, onClickListener).show().setCancelable(false);
    }
    ...
    private void autoStartOppo(final Context context) {
        System.out.println("isPackageExists");
